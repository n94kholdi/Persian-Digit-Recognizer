...
if __name__ == "__main__":
        ...
        adam = tf.keras.optimizers.Adam(lr=0.005, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.001, amsgrad=False)
        ...
        Checkpoint = tf.keras.callbacks.ModelCheckpoint("../Saved_models/Detector/myDetectorCNN.h5", monitor='val_accuracy', mode='max', verbose=0,
                                                     save_best_only=True, save_weights_only=False, period=1)

        early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_accuracy', min_delta=0, patience=5, verbose=0, mode='max')
        reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.2,
                                      patience=5, min_lr=0.0001)

        csv_logger = tf.keras.callbacks.CSVLogger('../Saved_models/Detector/log_detector.csv', append=True, separator=';')
        ...
        best_model = tf.keras.models.load_model("../Saved_models/Detector/myDetectorCNN.h5")
        ...
        pd.DataFrame(history).to_csv('../Saved_models/Detector/history.csv')
        ...
        plt.savefig("../Saved_models/Detector/accuracy.jpg")
        plt.clf()
        ...
        plt.xlabel('epoch')
        plt.legend(['train', 'test'], loc='upper left')
        plt.savefig("../Saved_models/Detector/loss.jpg")
