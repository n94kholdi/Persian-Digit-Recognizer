...
if __name__ == "__main__":
    ...
    sgd = tf.keras.optimizers.SGD(lr=0.005, decay=1e-7, momentum=0.9, nesterov=True)
    ...
    valid_loader = train_datagen.flow_from_directory(PROCESSED_VALID_PATH, batch_size=BATCH_SIZE,
                                                     target_size=INPUT_SIZE[:2])
    ...
    early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_accuracy', min_delta=0, patience=5, verbose=0,
                                                      mode='max')
    reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.2,
                                                     patience=5, min_lr=0.0001)
    csv_logger = tf.keras.callbacks.CSVLogger('../Saved_models/Classifierlog.csv', append=True, separator=';')
